        ...
        <div class="card mb-4">
            <div class="card-body bg-black">
                <div class="mb-2 d-flex align-items-end justify-content-between">
                    <h5 class="card-title">Data Transaction Package Group</h5>
                </div>
                            ...
                            @foreach ($resevasi as $no => $us)
                                <tr>
                                    ...
                                    <td class="text-white">Rp.
                                        {{ number_format($us->group->price, 2, '.', ',') }}</td>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('admin.transaksi.delete_group', $us->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                                        ...
                                        @elseif($us->status_dp == 'telah di sewa')
                                        <a href="{{ Route('admin.transaksi.dp_selesai_group', $us->id) }}" class="btn btn-success">Selesai</a>
                                        @elseif($us->status_dp == 'menunggu konfirmasi')
                                        <form action="{{ Route('transaksi.confirmation_dp_group', $us->id) }}" method="post">
                                            @csrf
                                            @method('put')
                                        ...
                                        <form action="{{ Route('admin.transaksi.dp_reject_group', $us->id) }}" method="post">
                                            @csrf
                                            @method('put')
