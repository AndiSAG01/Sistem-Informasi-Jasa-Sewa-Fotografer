...
class Resevasi_AqiFullController extends Controller
{
    public function index()
    {
        $resevasi = Resevasi_aqi::all();
        return view('admin.resevasi_aqi.fullpayment', compact('resevasi'));
    }
    ...
    public function end_dp($id)
    {
        ...
        if ($resevasi) {
            // Pastikan properti 'aqiqah' tersedia
            if ($resevasi->aqiqah) {
                $resevasi->aqiqah->id; // Akses properti 'aqiqah'
            } else {
                // Jika 'aqiqah' tidak ada, beri tahu pengguna
